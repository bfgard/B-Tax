...
def load_partner_data(entity_dfs):
    ...
    df05 = df05[['Corporate general partners','Corporate limited partners', 
    ...
    cross = pd.read_csv(_PRT_CROSS)
    df05 = pd.concat([cross,df05],axis=1)
    ...
    # # Not currently used except to count number of types
    ...
    '''
    corp = pd.merge(corp, soi_ind_codes, how='inner', left_on=['INDY_CD'], right_on=['minor_code_alt'], 
        left_index=False, right_index=False, sort=False, suffixes=('_x', '_y'), copy=True, indicator=True)
    # # keep only rows that match in both datasets - this should keep only unique soi minor industries
    corp = corp.drop(corp[corp['_merge']!='both' ].index)
    #
    corp_ratios = corp[['INDY_CD','minor_code_alt','minor_code','major_code','sector_code']]
    for var in corp_data_variables_of_interest :
        corp_ratios[var+'_ratio'] = corp.groupby(['sector_code'])[var].apply(lambda x: x/float(x.sum()))
    '''
